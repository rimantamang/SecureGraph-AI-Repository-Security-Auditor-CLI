...
from prompts.secrets_prompt import SECRETS_PROMPT
...
def analyze_secrets(file: dict) -> list[dict]:
    path = file["path"]
...
def run_secrets_agent(files: list[dict]) -> list[dict]:
    all_findings = []
    for file in files:
        findings = analyze_secrets(file)
        if findings:
            for f in findings:
